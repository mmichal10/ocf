...
struct ocf_cache_volume_io_priv {
	...
	env_atomic volume_remaining;
	env_atomic io_remaining;
	env_atomic error;
};
		...
		int error)
{
	...
	ocf_cache_log(cache, log_crit, "Cache volume io cmpl generic\n");

	priv = ocf_io_get_priv(io);

	if (env_atomic_dec_return(&priv->volume_remaining))
		return;
...
static void ocf_cache_io_complete(struct ocf_io *io, int error)
{
	...
	if (env_atomic_dec_return(&priv->io_remaining))
		return;

	//ocf_cache_log(cache, log_crit, "Finishing the IO");
...
static void ocf_cache_volume_submit_io(struct ocf_io *io)
{
	...
	env_atomic_set(&priv->volume_remaining, 2);
	env_atomic_set(&priv->io_remaining, 3);
...
static void ocf_cache_volume_submit_flush(struct ocf_io *io)
{
	...
	env_atomic_set(&priv->volume_remaining, 1);
