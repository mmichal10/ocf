...
static void _ocf_mngt_rebuild_metadata(ocf_cache_t cache, bool fix_metadata)
{
	ocf_cache_line_t cline;
	ocf_core_id_t core_id;
	uint64_t core_line;
...
static void _ocf_mngt_activate_check_superblock_complete(void *priv, int error)
{
	...
	if (cache->conf_meta->metadata_layout != cache->metadata.layout) {
		ocf_cache_log(cache, log_err, "Failed to activate failover instance: "
