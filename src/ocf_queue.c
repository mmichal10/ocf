...
ocf_queue_t ocf_queue_create(ocf_cache_t cache, uint32_t id)
{
	ocf_queue_t queue;
...
void ocf_queue_get(ocf_queue_t queue)
{
	...
	//printk(KERN_ERR "queue get id %d ref %d io count %d\n", queue->id,
	//	 env_atomic_read(&queue->ref_count), env_atomic_read(&queue->io_no));
...
void ocf_queue_put(ocf_queue_t queue)
{
	...
	printk(KERN_ERR "queue put id %d ref %d io count %d\n", queue->id,
		 env_atomic_read(&queue->ref_count), env_atomic_read(&queue->io_no));
