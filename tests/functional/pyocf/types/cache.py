...
class Cache:
    ...
    def __init__(
        ...
        self.cfg = CacheConfig(
            ...
            _cleaning_policy=cleaning_policy,
    ...
    def stop(self):
        ...
        c.wait()
        if c.results["error"]:
            ret = OcfErrorCode(-1 * c.results["error"])
        else:
            ret = OcfErrorCode.OCF_OK
        if ret != OcfErrorCode.OCF_OK and ret != OcfErrorCode.OCF_ERR_WRITE_CACHE:
            self.write_unlock()
